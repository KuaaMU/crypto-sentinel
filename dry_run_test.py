"""Dry-run test: 3 cycles on REAL market data, simulated orders."""
import asyncio
import logging
import sys
...
async def dry_run_test():
    ...
    # Create exchange in dry-run mode with $10,000 virtual balance
    ...
    print(f"\n{'='*60}")
    print(f"  DRY-RUN TEST (Real Data, Simulated Orders)")
    print(f"  Virtual Balance: ${balance:,.2f} USDT")
    print(f"  Pairs: {', '.join(config.exchange.pairs)}")
    print(f"  Interval: {config.collectors.interval_seconds}s")
    print(f"  Threshold: {config.trading.entry_conviction_threshold}")
                ...
                if atr > 0:
                    ...
                    try:
                        ...
                        if pos:
                            ...
                            print(f"  >>> DRY-RUN OPENED: id={pos.id} entry=${pos.entry_price:.2f} size={pos.size:.6f}")
                    except Exception as e:
    ...
    if isinstance(exchange, DryRunExchange):
        ...
        print(f"  Start: ${summary['initial_balance']:,.2f}")
        print(f"  Final: ${summary['final_balance']:,.2f}")
        print(f"  PnL: ${summary['pnl']:,.2f} ({summary['pnl_pct']:.2f}%)")
